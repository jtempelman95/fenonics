...
#################################################################
def solvesys(kx,ky,E,Mcomp,mpc,bcs,nvec):
    '''
    ===================================
...
#################################################################
def solve_bands(np1, np2, np3, c, rho, fspace, mesh):
    '''
    ===================================
    ...
    for kx in np.linspace(0.01,maxk,np1):
        K = fem.Constant(mesh, PETSc.ScalarType((kx,ky)))
        eval, evec = solvesys(kx,ky,E,Mcomp,mpc,bcs,nvec)
    ...
    return evals_disp, evec_all


#################################################################
#              Plot the mesh                                    #
#################################################################
def plotmesh(mesh,fspace):
    V = FunctionSpace(mesh,(fspace,1))
    v = Function(V)
...
if __name__ == "__main__":
    ...
    # Assign Mesh
    ###################################
    plotter = plotmesh(mesh,fspace)
    plotter.show()

   

    evals_disp, evec_all = solve_bands(np1, np2, np3, c, rho, fspace, mesh)

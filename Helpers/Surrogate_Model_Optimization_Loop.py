...
def load_data(MshRs,Xdim,refinement_level, Nquads, SamplrDim, Nsamp, LHS_Seed, path):
    ...
    for j in range(nfile):
        fname = str(j)+'.csv'
        dvec_dat = np.loadtxt(datapath+'//Dvecdata//'+files[j])
        splne_dat = np.loadtxt(datapath+'//Splinecurves//'+files[j], delimiter=',')
        BG_dat   = np.loadtxt(datapath+'//BGdata//'+files[j], delimiter=',')
        Band_dat = np.loadtxt(datapath+'//dispersiondata//'+files[j], delimiter=',')
...
if __name__ == '__main__':
    ...
    #################################################################
    fstar_FEM_raw = objfun(np.array(evals_disp_eval), BG_op = BG_op)
    ...
    ######################################################################
    for Enrichment_Index in range(10):
        # -------------------------------------------
        ...
        # ----------------------------------------------
        if    np.max(targets_enriched)  < fstar_FEM: mean = out.x

...
datapath = ('data//TrainingData//SampleSeed '  + str(LHS_Seed) +'| SampleDim '+  str(sample.shape[1])  
...
print('======== COMPUTING BAND STRUCTURE WITH LHS SAMPLINE =========')
# =================================
...
for sidx in range(Nsamp):
    ...
    # r           = np.array([dvar1,dvar2,dvar3])*a_len*.9
...
# %matplotlib qt
lbmx = []
for iter in range(len(LBall)):
    lbuse = LBall[iter]
    if not np.any(lbuse):
        lbmx.append(0)
    else:
        lbmx.append(np.max(lbuse))
lu = len(BGnrm)

target_use = BGnrm
target_use = lbmx

plt.scatter(sample[0:lu,0],sample[0:lu,1], c =(np.array(target_use)), cmap = "jet")
plt.colorbar()
plt.show()

fig = plt.figure()
ax = plt.axes(projection='3d')
s1= ax.scatter(sample[:,0],sample[:,1],sample[:,2], c = (target_use), cmap = "jet")
plt.colorbar(s1)
plt.show()
...
features = sample
targets = np.array(target_use)
regressor.fit(features,targets)

xtest  = np.linspace(0,1,10)
ytest  = np.linspace(0,1,10)
ztest  = np.linspace(0,1,10)


Xtest,Ytest,Ztest = np.meshgrid(xtest,ytest,ztest)
Xtst = Xtest.reshape(10**3,1)
Ytst = Ytest.reshape(10**3,1)
Ztst = Ztest.reshape(10**3,1)
...
features = sample
targets = np.array(BGnrm)
regressor.fit(features,targets)

xtest  = np.linspace(0,1,20)
ytest  = np.linspace(0,1,20)
Xtest,Ytest = np.meshgrid(xtest,ytest)
testpt = y_pred = regressor.predict(features)
Xtst = Xtest.reshape(2500,1)
Ytst = Ytest.reshape(2500,1)
testdat = np.hstack( ( Xtst,Ytst ))
testpt = y_pred = regressor.predict(testdat )


plt.scatter(Xtst,Ytst, c =(testpt), cmap = "jet")
# plt.scatter(sample[0:lu,0],sample[0:lu,1], c =(testpt), cmap = "jet")
plt.colorbar()
plt.show()
...
# fig = plt.figure()
# ax = plt.axes(projection='3d')
# s1= ax.scatter(sample[:,0],sample[:,1],sample[:,2], c = np.log(BGnrm), cmap = "jet")
# plt.colorbar(s1)
# plt.show()
...
# ax.scatter(sample[:,0],sample[:,1], c = np.log(BGnrm), cmap = "jet")
# ax=plt.subplot(1,3,2)
# ax.scatter(sample[:,0],sample[:,2], c = np.log(BGnrm), cmap = "jet")
# ax=plt.subplot(1,3,3)
# ax.scatter(sample[:,1],sample[:,2], c = np.log(BGnrm), cmap = "jet")
...
# =================================
from scipy.stats import qmc
sampler = qmc.LatinHypercube(d=4,seed= 11)

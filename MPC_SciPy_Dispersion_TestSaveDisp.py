...
LHS_Seed    = 5
sampler     = qmc.LatinHypercube(d=4,seed =  LHS_Seed)
Nsamp       = 50
...
###########################################################
#  Scratch work - playing with pandas and gmsh
###########################################################
...
# dataframe = pd.DataFrame(columns=["Dispersoin"],
                        #  data = disp_info[:,0])
# dataframe = dataframe.append(disp_info[:,1],
                            #  ignore_index =True)
gmsh.finalize()           
gmsh.initialize()
gmsh.open((datapath+'//meshdata//'+str(sidx)+'.msh'))
###########################################################
# Get spline fit from the mesh 
###########################################################
node_interior = gmsh.model.mesh.getNodesForPhysicalGroup(1,5)[1]

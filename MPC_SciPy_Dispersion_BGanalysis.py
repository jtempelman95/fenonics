...
for j in range(len(gapwidths)):
    ...
    ub = highbounds[j]
    if j == 0:
        currentAxis.add_patch( Rectangle((np.min(xx),lb), np.max(xx), ub-lb,  facecolor="k" ,ec='w', alpha =.2,label='bangap'))
    else:
        currentAxis.add_patch( Rectangle((np.min(xx),lb), np.max(xx), ub-lb,  facecolor="k" ,ec='w', alpha =.2))
plt.legend()
# plt.savefig('Dispersion.pdf')
# plt.savefig('BandGapDgm.pdf', bbox_inches = 'tight')
plt.show()
...
for j in range(len(gapwidths)):
    ...
    ub = highbounds[j]
    if j == 0:
        ax1.add_patch( Rectangle((np.min(xx),lb), np.max(xx), ub-lb,  facecolor="k" ,ec='w', alpha =.2,label='bangap'))
    else:
        ax1.add_patch( Rectangle((np.min(xx),lb), np.max(xx), ub-lb,  facecolor="k" ,ec='w', alpha =.2))
ax1.legend()


ax2 = fig.add_subplot(221)
# ax2.add_patch( Rectangle( (-a_len/2,-a_len/2),a_len,a_len, facecolor="w" ,ec='k', alpha =.2,label='bangap'))
ax2.add_patch( Rectangle( (-a_len/2,-a_len/2),a_len,a_len, facecolor="none" ,ec='k', alpha =1,label='bangap'))
ax2.plot(x[0:int(len(x)/Nquads)], y[0:int(len(x)/Nquads)], ' .r')
ax2.plot(x[int(len(x)/Nquads):-1], y[int(len(x)/Nquads):-1], 'g.')
# ax2.plot(xi, yi, '-b')
...
ax2.add_patch( Rectangle( (-a_len/2,-a_len/2),a_len,a_len, facecolor="none" ,ec='k', alpha =1,label='bangap'))
ax2.plot(xi, yi, '-b')
ax2.set_title('Void Topology',fontsize = 18)
ax2.set_xlabel('$x$ [m]',fontsize = 18)
ax2.set_ylabel('$y$ [m]',fontsize = 18)
ax2.set_aspect('equal', 'box')
ax2.set_xlim((-a_len/1.5,a_len/1.5))
...
# plt.savefig('BandGapDgm_withDesign_opt.pdf', bbox_inches = 'tight') 
plt.show()
...
ax = plt.gca()
ax2 = ax.inset_axes([0.7, 0.7, 0.3, 0.3])
...
for r in range(len(x)-1):
    if r<int(len(x)/Nquads):
        ...
        ax2.plot([0,x[r]], [0,y[r]],'g--')
ax2.set_aspect('equal', 'box')
ax2.set_axis_off()

ax = plt.gca()
ax2 = ax.inset_axes([0.7, 0.4, 0.3, 0.3])
ax2.add_patch( Rectangle( (-a_len/2,-a_len/2),a_len,a_len, facecolor="w" ,ec='w', alpha =.75,label='bangap'))
ax2.add_patch( Rectangle( (-a_len/2,-a_len/2),a_len,a_len, facecolor="none" ,ec='k', alpha =1,label='bangap'))
ax2.plot(xi, yi, '-k')
ax2.set_aspect('equal', 'box')
ax2.set_axis_off()

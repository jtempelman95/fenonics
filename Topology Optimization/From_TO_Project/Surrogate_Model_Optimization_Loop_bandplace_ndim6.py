...
def multi_objfun(bands, BP_op = 5, BP_op2 = 5):
    '''
    Computing an objective function from a band structure
    '''
    bgnrm, gapwidths, gaps, lowbounds, highbounds = getbands(bands)
    ...
    # ==============================
    # quantity    =  gapwidths
    # opt1        =  objfnc_bgloc(lowbounds,highbounds,quantity,gaploc =BP_op,  centered = True)
    # opt2        =  objfnc_bgloc(lowbounds,highbounds,quantity,gaploc =BP_op2, centered = True)
    # ==============================
    # To check if bandgap is above/below 
    # ==============================
    # quantity    =  bgnrm
    # opt1   =  objfnc_bglowboundsHighLow(,highbounds, quantity , gaploc = 1.5e3,   below = True)
    # opt2   =  objfnc_bgHighLow(lowbounds,highbounds, quantity , gaploc = 3.5e3,    below = False)
    # ==============================
    # To check band gaps within freq ranges
    # ==============================
    # optvec[k]  =  -objfnc_bgrng(lowbounds,highbounds,gaploc_lower = 1e2,gaploc_upper = 1e3,  max = False )
    opt1  =   objfnc_bgrng(lowbounds,highbounds,gaploc_lower = BP_op, gaploc_upper = BP_op+500,         max = False)
    opt2 =    objfnc_bgrng(lowbounds,highbounds,gaploc_lower = BP_op2, gaploc_upper = BP_op+1e3,        max = False)
    # ==============================
    # Adhoc definition
    # ==============================
    # if any(lowbounds):
...
if __name__ == '__main__':
    ...
    #################################################
        ...
        figpath     = ('..//figures//Optimization//Quads_'      +  str(Nquads)  + ' Xdim_' 
                                                                +  str(Xdim)    +  ' MshRs_'     + str(MshRs)
                                                                + ' rfnmt_'     +  str(rf_level)
        ...
        inds_keep = targets > .15
        inds_keep = [i for i, x in enumerate(inds_keep) if x]
        dvecs2    = [dvecs[inds]    for inds in inds_keep]  ; dvecs = dvecs2
        Bands2    = [Bands[inds]    for inds in inds_keep]  ; Bands = Bands2
        splines2  = [splines[inds]  for inds in inds_keep]  ; splines = splines2
        features = features[inds_keep,:]
        targets  = targets[inds_keep]
        ...
        #################################################
        BATCH_SIZE  = 36           # int(len(features)/8
        BATCH_SIZE  = int(BATCH_SIZE)
        fair        = True
        #################################################
        x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=50)
        x_train     = torch.Tensor(x_train).float()
        y_train     = torch.Tensor(y_train).float().flatten()
        if not fair:
        ...
        #################################################
        model       = Net(dimx, dimy, First = 64 , deep  = 64, Ndeep = 4, isrelu = True) .to(device)
        loss_fn     = nn.MSELoss()
        loss_fn     = CustomLoss()
        ...
        if showplots:
            ...
            if savefigs: plt.savefig(figpath+'//Error'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
            plt.show()
            predictions = model(x_train)
            plt = riseplot(targets.flatten(),predictions.detach().flatten())
            if savefigs: plt.savefig(figpath+'//RisePlotNN'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
            plt.show()
            plt = targetplot(targets.flatten(),predictions.detach().flatten())
            if savefigs: plt.savefig(figpath+'//TargetPlotNN'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
            plt.show()
        ...
        if showplots:
            if x_train.shape[1] == 3:
                ...
                plt.colorbar(s1)
                if savefigs: plt.savefig(figpath+'//Targets'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
                fig     = plt.figure(figsize = (5,4))
                ax      = plt.axes(projection='3d')
                ...
                plt.colorbar(s1)
                if savefigs: plt.savefig(figpath+'//FeatPred_NN'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
                fig     =   plt.figure(figsize = (5,4))
                ax      =   plt.axes(projection='3d')
                ...
                if savefigs: plt.savefig(figpath+'//GridPred_NN'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
                ...
                plt.xlabel('Design Feature 1')
                plt.ylabel('Design Feature 2')
                if savefigs: plt.savefig(figpath+'//ContourNN'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
        ...
        def optfun(x):
            feval  = model(torch.Tensor(x)).detach().numpy()
            return -(feval)
        ...
        if features.shape[1] ==3:
            ...
            if showplots:
                ...
                s1=ax.scatter(testdat[:,0],testdat[:,1],testdat[:,2],alpha = .095, c = cdata, cmap = 'twilight_shifted')
                ax.view_init(elev=20, azim=80, roll=0)    
                plt.colorbar(s1)
                ax.set_title('Neural Network Over Grid')
            ...
            if showplots:
                if savefigs: 
                    plt.savefig(figpath+'//OptPlot'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
        if features.shape[1] ==2:
            ...
            for k in range(10):
                ...
                if showplots:
                    ...
                    plt.scatter(xopt[0],xopt[1],color=(.2,.8,.2), marker='o', s= 20,alpha = .45)
            if showplots: 
                if savefigs: 
                    plt.savefig(figpath+'//OptPlot'+str(Enrichment_Index)+'.pdf', bbox_inches = 'tight')
                plt.show()
        ...
        meshalg_eval                    =   6
        #########################################################
        #            Define the design vec get model           #
        #########################################################
        design_vec_eval         = np.concatenate( (r_eval/a_len_eval, [offset_eval] ))
        gmsh.model, xpt, ypt    = get_mesh_SquareSpline(a_len_eval,da_eval,r_eval,Nquads_eval,offset_eval,meshalg_eval, refinement_level_eval,refinement_dist_eval,isrefined = True, cut = True)
        ...
        if showplots:
            ...
            plotter.show()
        #########################################################
        #  Solve the FEM problem and post-process results       #
        #########################################################
        evals_disp_eval, evec_all_eval = solve_bands(np1, np2, np3,nvec, a_len_eval, c, rho, fspace, mesh, ct)
        disp_info_eval   = np.array(evals_disp_eval)
        bgnrm_eval, gapwidths_eval, gaps_eval, lowbounds_eval, highbounds_eval = getbands(np.array(evals_disp_eval))
        SplinePtDat_eval,SplineDat_eval = GetSpline(gmsh)
        #################################################################
        #               Evaluate the predicted output                   #
        #################################################################
        fstar_FEM_raw = multi_objfun(np.array(evals_disp_eval), BP_op = BP_op,BP_op2 = BP_op2)
        fstar_FEM =   normalize(fstar_FEM_raw,targ_raw)
        ...
        #################################################################
        #         Plot Predicted vs Recorded Optimal                    #
        #################################################################
        if showplots:
            plt.figure(figsize=(5,5))
            plt.plot(splines[targets.argsort()[-1]][:,0],splines[targets.argsort()[-1]][:,1], label = 'ad-hoc' ); 
            plt.plot(SplineDat_eval[:,0],SplineDat_eval[:,1],label='NN Optimal' ); 
            plt.legend()
            plt.axis('equal')
        ...
        #################################################################
        #               Save the 'optimal' outputs                      #
        #################################################################
        datapath_optimal = datapath[0:10] + '//OptimizedData' +datapath[22::]+  '//BGop_'+str(BP_op)+'BP_op2'+str(BP_op2)+'weighted'
        if not os.path.exists(datapath_optimal):
                        os.makedirs(datapath_optimal+'//dispersiondata')
                        os.makedirs(datapath_optimal+'//meshdata')
                        os.makedirs(datapath_optimal+'//Splinecurves')
                        os.makedirs(datapath_optimal+'//BGdata')
                        os.makedirs(datapath_optimal+'//Dvecdata')
                        os.makedirs(datapath_optimal+'//Splinepts')
        ngaps       = len(gapwidths_eval)
        if ngaps    == 0:   BGdata_eval = np.zeros(4)
        else:               BGdata_eval = np.hstack((gapwidths_eval.reshape(ngaps,1),lowbounds_eval.reshape(ngaps,1),
                            highbounds_eval.reshape(ngaps,1),bgnrm_eval.reshape(ngaps,1)))
        if savedata:
            np.savetxt((datapath_optimal + '//Dvecdata//NN'      +str(Enrichment_Index)+'.csv'),     design_vec_eval,     delimiter=",")
            np.savetxt((datapath_optimal + '//BGdata//NN'        +str(Enrichment_Index)+'.csv'),     BGdata_eval,         delimiter=",")
            np.savetxt((datapath_optimal + '//dispersiondata//NN'+str(Enrichment_Index)+'.csv'),     disp_info_eval,      delimiter=",")
            np.savetxt((datapath_optimal + '//Splinecurves//NN'  +str(Enrichment_Index)+'.csv'),     SplineDat_eval,      delimiter=",")
            np.savetxt((datapath_optimal + '//Splinepts//NN'     +str(Enrichment_Index)+'.csv'),     SplinePtDat_eval,    delimiter=",")
            gmsh.write((datapath_optimal + '//meshdata//NN'      +str(Enrichment_Index)+'.msh'))

            optarg = np.argmax(targets)
            np.savetxt((datapath_optimal + '//Dvecdata//sampled'      +str(Enrichment_Index)+'.csv'),    dvecs[optarg],         delimiter=",")
            np.savetxt((datapath_optimal + '//dispersiondata//sampled'+str(Enrichment_Index)+'.csv'),    Bands[optarg],         delimiter=",")
            np.savetxt((datapath_optimal + '//Splinecurves//sampled'  +str(Enrichment_Index)+'.csv'),    splines[optarg],       delimiter=",")
        ...
        =================================================================================================                
                                    ...
                                    ENRICHMENT SAMPLING OPTIMIZATION LOOP
        ...
        ######################################################################
        #            OUTTER LOOP - ENRICHEMENT BATCH INDEX                   #
        ######################################################################
        for Enrichment_Index in range(12):
            # -------------------------------------------
            # Initalize/update Enrichement Variables
            # -------------------------------------------
            epsilon_all.append(epsilon)
            fstar_all.append(fstar_FEM)
            fstar_nn_all.append(-out.fun)
            fstar_raw_all.append(fstar_FEM_raw)
            if Enrichment_Index == 0:
                features_enriched   = features
                targets_enriched    = targets
                splines_enriched    = splines
                Bands_enriched      = Bands
                dvecs_enriched      = dvecs
            splines_enriched.append(SplineDat_eval)
            Bands_enriched.append(disp_info_eval)
            dvecs_enriched.append(design_vec_eval)
            features_enriched   = np.vstack((features_enriched,     r_eval))
            targets_enriched    = np.concatenate((targets_enriched,  fstar_FEM.reshape(1,)))
            # ----------------------------------------------
            # Generate Samples from Mulit-variate Gaussain
            # ----------------------------------------------
            if    np.max(targets)  < fstar_FEM: mean = out.x
            else: mean = dvecs_enriched[targets_enriched.argsort()[-1]][0:-1]
            cov = np.eye(len(mean))/300
            if len(mean) ==6:
                ...
                xs1, xs2 = np.random.multivariate_normal(mean, cov, 100).T
                xsall = np.vstack((xs1,xs2))
                xsall[xsall<.1]  = .1
                xsall[xsall>.95] = .95
            # ----------------------------------------------
            # Initialize Path to save Enrichment Samples
            # ----------------------------------------------
            datapath_enrich = datapath[0:10] + '//EnrichementData' +datapath[22::]+  '//BGop_'+str(BP_op) +'BP_op2'+str(BP_op2)+'weighted' +'//enrich_loop' + str(Enrichment_Index)
            if savedata:
            ...
            ######################################################################
            #            INNER LOOP - ENRICHEMENT SAMPLE INDEX                   #
            ######################################################################
            for sidx in range(5):
                ...
                gmsh.model, xpt, ypt    = get_mesh_SquareSpline(a_len_eval,da_eval,r_enrich,Nquads_eval,offset_eval,meshalg_eval, refinement_level_eval,refinement_dist_eval,isrefined = True, cut = True)
                ...
                # ----------------------------------------------
                # Solve the FEM  problem and post-process
                # ----------------------------------------------
                ...
                SplinePtDat_enrich, SplineDat_enrich = GetSpline(gmsh)
                ...
                # ----------------------------------------------
                # Save the output of the enrichment sample
                # ----------------------------------------------
                if savedata:
                ...
                # ----------------------------------------------
                # Update inner-loop enrichement variables
                # ----------------------------------------------
                target_enrichment   = normalize(multi_objfun(np.array(evals_disp_enrich), BP_op = BP_op,BP_op2 = BP_op2), targ_raw)
                ...
                dvecs_enriched.append(design_vec_enrich)
                Bands_enriched.append(disp_info_enrich)
                splines_enriched.append(SplineDat_enrich)

            ######################################################################
            #            RETRAIN THE NEURAL NETWORK WITH NEW SAMPLES             #
            ######################################################################
            BATCH_SIZE  = 32
            fair    = True
            x       = features_enriched
            y       = targets_enriched.reshape((len(targets_enriched),1))
            x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.2, random_state=50)
            x_train     = torch.Tensor(x_train).float()
            ...
            Nparams     = sum(p.numel() for p in model.parameters() if p.requires_grad)
            model, history, historyt =   trainingloop(model,optimizer,x_train,y_train,x_test,y_test,dataloader_train, 
                                        fair = fair,  patience = 15, min_delta = 1e-6, restore_best_weights = True)

            ######################################################################
            #                 EVALUATE FIT OF RETRAINED NETWORK                  #
            ######################################################################
            x_train     = torch.Tensor(x).float()
            y_train     = torch.Tensor(y).float()
            pred = model(x_train)
            ...
            if showplots:
                plt = learningplot(historyt,history)
                if savefigs: plt.savefig(figpath+'//Error'+str(Enrichment_Index+1)+'.pdf', bbox_inches = 'tight')
                plt.show()
                plt = riseplot(targets_enriched.flatten(),predictions.detach().flatten())
                if savefigs: plt.savefig(figpath+'//RisePlotNN'+str(Enrichment_Index+1)+'.pdf', bbox_inches = 'tight')
                plt.show()
                plt = targetplot(targets_enriched.flatten(),predictions.detach().flatten())
                if savefigs: plt.savefig(figpath+'//TargetPlotNN'+str(Enrichment_Index+1)+'.pdf', bbox_inches = 'tight')
                plt.show()
            
            ######################################################################
            #               OPTIMIZATION OF RETRAINED NETWORK                    #
            ######################################################################
            if features.shape[1] == 3:
                bnds = ((0.1, .95), (0.1, .95), (0.1, .95))
                fstar = 1e5
            ...
            ######################################################################
            #            GENERATE FEM DISPERSION ON PREDICTED OPIMAL             #
            ######################################################################
            ...
            bgnrm_eval, gapwidths_eval, gaps_eval, lowbounds_eval, highbounds_eval = getbands(np.array(evals_disp_eval))
            #################################################################
            #          EVALUATE THE MATCH OF SURROGATE MODEL                #
            #################################################################
            fstar_FEM_raw = multi_objfun(np.array(evals_disp_eval), BP_op = BP_op, BP_op2 = BP_op2)
            fstar_FEM =   normalize(fstar_FEM_raw,targ_raw)
            epsilon = np.abs(fstar_FEM- np.abs(out.fun))/( fstar_FEM)
            ...
            #                Save the optimal outputs                       #
            #################################################################
            np.savetxt((datapath_optimal + '//Dvecdata//NN'      +str(Enrichment_Index + 1)+'.csv'),     design_vec_eval,     delimiter=",")
            np.savetxt((datapath_optimal + '//BGdata//NN'        +str(Enrichment_Index + 1)+'.csv'),     BGdata_eval,         delimiter=",")
            np.savetxt((datapath_optimal + '//dispersiondata//NN'+str(Enrichment_Index + 1)+'.csv'),     disp_info_eval,      delimiter=",")
            np.savetxt((datapath_optimal + '//Splinecurves//NN'  +str(Enrichment_Index + 1)+'.csv'),     SplineDat_eval,      delimiter=",")
            np.savetxt((datapath_optimal + '//Splinepts//NN'     +str(Enrichment_Index + 1)+'.csv'),     SplinePtDat_eval,    delimiter=",")
            gmsh.write((datapath_optimal + '//meshdata//NN'      +str(Enrichment_Index + 1)+'.msh'))

            optarg = np.argmax(targets_enriched)
            np.savetxt((datapath_optimal + '//Dvecdata//sampled'      +str(Enrichment_Index + 1)+'.csv'),     dvecs_enriched[optarg],                   delimiter=",")
            np.savetxt((datapath_optimal + '//dispersiondata//sampled'        +str(Enrichment_Index + 1)+'.csv'),     Bands_enriched[optarg],           delimiter=",")
            np.savetxt((datapath_optimal + '//Splinecurves//sampled'  +str(Enrichment_Index + 1)+'.csv'),     splines_enriched[optarg],                 delimiter=",")
            
        #################################################################
        #                Save Optimization Histor                       #
        #################################################################
        np.savetxt((datapath_optimal + '//FEM_star_raw.csv'),   fstar_raw_all ,                     delimiter=",")
        np.savetxt((datapath_optimal + '//FEM_star.csv'),   fstar_all ,                     delimiter=",")
        np.savetxt((datapath_optimal + '//NN_star.csv'),    fstar_nn_all ,                  delimiter=",")
        np.savetxt((datapath_optimal + '//epsilon.csv'),    epsilon_all ,                   delimiter=",")
        np.savetxt((datapath_optimal + '//targets_enriched.csv'),  targets_enriched ,       delimiter=",")
        np.savetxt((datapath_optimal + '//features_enriched.csv'),  features_enriched ,     delimiter=",")


        if showplots:
            plt.figure(figsize=(3,3))
            plt.hist(targets, alpha = .5,label = 'LHS Samples') 
            plt.hist(targets_enriched[targets.shape[0]::] ,alpha = .5,facecolor = 'r',label = 'Enrichment')
            plt.xlabel('Target Val')
            plt.ylabel('Target Val')
            plt.legend()
            if savefigs: plt.savefig(figpath+'//TargetPlot_Enrichement.pdf', bbox_inches = 'tight')
            plt.show()
            ...
            ax2.hist(targets_enriched[targets.shape[0]::] ,alpha = .5,facecolor = 'g',label = 'Enrichment')
            ax2.tick_params(axis ='y', labelcolor = color)
            plt.legend(loc=(.65,1.05))
            # plt.grid()
            if savefigs: plt.savefig(figpath+'//TargetPlot_Enrichement2.pdf', bbox_inches = 'tight')
            plt.show()
            
            plt.figure(figsize=(6,3))
            plt.subplots_adjust(left    =0.1,bottom  =0.1,right   =0.9,top     =0.9, wspace  =0.35, hspace  =0.15)
            plt.subplot(121)
            plt.plot(epsilon_all,'-o',markersize=10)
            plt.xlabel('Iteration')
            plt.ylabel('$\epsilon$')
            plt.subplot(122)
            plt.plot(fstar_all,'-o',markersize=10)
            plt.xlabel('Iteration')
            plt.ylabel('$f(x)$')
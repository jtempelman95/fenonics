...
from Surrogate_Model_Optimization_Loop import *
savefigs = 1
if __name__ == '__main__':
    ...
    f_optimal       = []
    for k in range(13):
        datapath_enrich = datapath[0:10] + '//EnrichementData' +datapath[22::]+  '//BGop_'+str(BG_op) +'//enrich_loop' + str(k)
        ...
        if os.path.exists(datapath_enrich):
            ...
            for j in range(nfile):
                ...
                fraw_enrich  = objfun(np.array(Band_dat), BG_op = BG_op)
    ...
    for j in range(nfile):
        ...
        if os.path.exists(datapath_optimal+'//Dvecdata//'+fname):
            ...
            fraw_optimal  = objfun(np.array(Band_dat), BG_op = BG_op)
...
# plt.hlines(np.max(Bands_optimal[optuse][:,BG_op-1]),0,np.max(xx),'r')
# plt.hlines(np.min(Bands_optimal[optuse][:,BG_op]),0,60,'r')
# plt.hlines(np.max(Bands[np.argsort(targets)[-1]][:,BG_op-1]),0,60,'b')
# plt.hlines(np.min(Bands[np.argsort(targets)[-1]][:,BG_op]),0,60,'b')

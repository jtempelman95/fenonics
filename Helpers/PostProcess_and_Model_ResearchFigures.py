...
figpath = ('figures//WeeklyMeetings//Quads_'    + str(Nquads) + ' Xdim_' 
                                                + str(Xdim)    +  ' MshRs_'+ str(MshRs)
                                                + ' rfnmt_' +  str(refinement_level) )
...
for k in range(len(BGdata)):
    ...
    if any(lowbounds):
        for j,lb in enumerate(lowbounds):
            if lb < gaploc and highbounds[j]>gaploc:
                optfnc = gapwidths[j]
    else:
        optfnc      = 0
...
# features    = np.array(Rads)
targets     = (optvec2/np.max(optvec2)/2 + optvec/np.max(optvec))
# targets   = optvec2
...
#################################################
scaler = MinMaxScaler()
# x  = scaler.fit_transform(features)-.5
x = features
# x = np.array(Rads)
y = (targets)
...
#################################################
BATCH_SIZE = 60 # int(len(features)/8)
#################################################
x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=0.00025, random_state=42)
...
if x_train.shape[1] == 3:
    ...
    fig     = plt.figure(figsize = (5,4))
    ax      = plt.axes(projection='3d')
    s1=ax.scatter(testdat[:,0],testdat[:,1],testdat[:,2],alpha = .15, c = pred_grid.flatten().cpu().detach(),  cmap = 'twilight_shifted')
    ax.view_init(elev=20, azim=80, roll=0)    
    plt.colorbar(s1)
    # plt.colorbar(s1)

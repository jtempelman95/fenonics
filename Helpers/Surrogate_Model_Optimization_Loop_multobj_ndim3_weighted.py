...
def multi_objfun(bands, BG_op = 5, BG_op2 = 5):
    ...
    # optvec[k]  =  np.max(np.diff(bands[:, 4]))
    # optvec2[k]  = np.max(np.diff(bands[:, 4]))
    return opt1*np.sqrt(B1) + opt2*np.sqrt(B2)
...
if __name__ == '__main__':
    ...
    #################################################
        ...
        for Enrichment_Index in range(12):
            ...
            # ----------------------------------------------
            datapath_enrich = datapath[0:10] + '//EnrichementData' +datapath[22::]+  '//BGop_'+str(BG_op) +'BG_op2'+str(BG_op2)+'weighted' +'//enrich_loop' + str(Enrichment_Index)
            if savedata:

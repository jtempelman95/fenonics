...
if __name__ == '__main__':
    ...
    figpath     = ('..//figures//Optimization//Examples//Quads_'        +  str(Nquads)  + ' Xdim_' 
                                                                        +  str(Xdim)    +  ' MshRs_'     + str(MshRs)
                                                                        + ' rfnmt_'     +  str(rf_level)
                                                                        +'//BG_op'+str(BG_op))
    ...
    if showplots:
        if x_train.shape[1] == 3:
            ...
            s1      =   ax.scatter(testdat[:,0],testdat[:,1],testdat[:,2],alpha = .25, c = pred_grid.flatten().cpu().detach(),  cmap = 'twilight_shifted')
            ax.view_init(elev=20, azim=80, roll=0)    
            plt.colorbar(s1)

...
def objfun(bands, BG_op = 5, BG_op2 = 5):
    '''
    Computing an objective function from a band structure
    '''
    bgnrm, gapwidths, gaps, lowbounds, highbounds = getbands(bands)
...
if __name__ == '__main__':
    ...
    #################################################
        ...
        for k in range(len(BGdata)):
            ...
            '''
            optvec[k]   = objfun(Bands[k], BG_op = BG_op, BG_op2 = BG_op2)
            xvec[k,0]   = dvecs[k][0]
            xvec[k,1]   = dvecs[k][1]
            if len(xvec[0,:]) > 2:
                xvec[k,2]   = dvecs[k][2]
        ...
        #################################################################
        fstar_FEM_raw = objfun(np.array(evals_disp_eval), BG_op = BG_op,BG_op2 = BG_op2)
        ...
        #################################################################
        datapath_optimal = datapath[0:10] + '//OptimizedData' +datapath[22::]+  '//BGop_'+str(BG_op)
        ...
        for Enrichment_Index in range(12):
            ...
            for sidx in range(5):
                ...
                # ----------------------------------------------
                target_enrichment   = normalize(objfun(np.array(evals_disp_enrich), BG_op = BG_op,BG_op2 = BG_op2), targ_raw)

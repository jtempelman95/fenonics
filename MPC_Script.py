...
a = 1
da = a/100
r =  a*np.array([1.5,.1,0,.1])*1
r =  a*np.array([.5,.25,.5])*1.5
offset = 0*np.pi/4
Nquads = 4
meshalg = 6
refinement_level = 3
refinement_dist = a/20
...
plotter.screenshot('Test.png',window_size=[1300,1300])
plotter.show()
...
plotter = pyvista.Plotter(shape=(2, 2), window_size=(1200,800))
R  = 3;
mycmap = plt.cm.get_cmap('Set2')
mycmap = plt.cm.get_cmap('seismic', 20)
...
ax2.plot(ybcl-ybcr,'b',label = 'LR Boundary Node')
ax2.plot(xbct-xbcb,'r--',label = 'TB Boundary Node')
